#!/usr/bin/env node
// build_docs.mjs — build csarko.sh's docs pages from docs/published/*.md, /games from
// docs/games/*.md and /film from docs/films/*.md.
//
...
import { buildSite, DocError, DOCS_DIR, FILM_DIR } from './docs_lib.mjs';

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '../../../..');
const PUBLIC = join(ROOT, 'public');
const CONTENT = join(ROOT, 'docs/published');
const GAMES = join(ROOT, 'docs/games');
const FILMS = join(ROOT, 'docs/films');
...
const filmSources = read(FILMS);

let files;
try {
  files = buildSite({ sources, gameSources, filmSources, indexHtml: readFileSync(join(PUBLIC, 'index.html'), 'utf8') });
} catch (e) {
...
if (out === PUBLIC) {
  for (const dir of [DOCS_DIR, 'games', FILM_DIR]) {
...
const extra = [files.has('games/index.html') ? `${gameSources.length} game(s)` : null,
  files.has(`${FILM_DIR}/index.html`) ? `${filmSources.length} film(s)` : null].filter(Boolean);
console.log(`docs: ${pages} page(s)${extra.length ? ` + ${extra.join(' + ')}` : ''} → ${where}`);
