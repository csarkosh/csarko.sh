...
`google-auth` to mint a token and stdlib urllib for the calls. No other
dependency, so the script is portable to any static-site repo.
...
Setup, quotas and what stays manual: .agents/skills/search-console/SKILL.md
"""
...
def missing_key_message(path):
    return (
        ...
        "          .agents/skills/search-console/scripts/setup.sh --project <gcp-project>\n"
...
def access_token(path):
    ...
    try:
        ...
        raise Failure("google-auth is not installed: python3 -m pip install google-auth")
    try:
...
def cmd_sitemap(opts, token):
    ...
    if not result.get("lastDownloaded"):
        warn("Google has never downloaded this sitemap. If it stays this way for several days, "
             "delete it in Search Console > Sitemaps and submit it once more. See AGENTS.md "
             "'Search Console state' before re-doing it.")
    else:
